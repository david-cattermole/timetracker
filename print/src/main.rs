...
fn create_presets(settings: &PrintAppSettings) -> Result<(Vec<PrintPresetSettings>, Vec<String>)> {
    ...
    for preset_name in &settings.print.display_presets {
        let preset = match settings.print.presets.get(&preset_name.clone()) {
            Some(value) => {
...
fn main() -> Result<()> {
    ...
    let (presets, missing_preset_names) = create_presets(&settings)?;
